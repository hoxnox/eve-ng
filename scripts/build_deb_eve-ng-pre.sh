...
VERSION="$(cat ${SRC_DIR}/VERSION.PRE 2>> ${LOG} | cut -d- -f1 2>> ${LOG})"
RELEASE="$(cat ${SRC_DIR}/VERSION.PRE 2>> ${LOG} | cut -d- -f2 2>> ${LOG})"
...
sed -i 's/.*GRUB_CMDLINE_LINUX=.*/GRUB_CMDLINE_LINUX="net.ifnames=0 intel_iommu=on isolcpus=1-99 nohz_full=1-99 rcu_nocbs=1-99 '"\$BAREPARAMS"' "/g' /etc/default/grub &> /dev/null
